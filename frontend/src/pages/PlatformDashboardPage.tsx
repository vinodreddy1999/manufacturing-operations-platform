    ...
    <div className="space-y-6">
      ...
      <Panel title="Module Health" description={moduleHealthDescription}>
        <div className="mb-4 grid gap-3 lg:grid-cols-[minmax(220px,1fr)_220px_280px]">
          <label className="text-sm text-slate-400"><span className="mb-1 block">Search module</span><input className="form-input w-full py-2" type="search" placeholder="Search by module name..." value={moduleSearch} onChange={(event) => setModuleSearch(event.target.value)} /></label>
          <label className="text-sm text-slate-400"><span className="mb-1 block">Availability</span><select className="form-input w-full py-2" value={moduleAvailability} onChange={(event) => setModuleAvailability(event.target.value as 'all' | 'enabled' | 'disabled')}><option value="all">All statuses</option><option value="enabled">Enabled</option><option value="disabled">Disabled</option></select></label>
          <label className="text-sm text-slate-400"><span className="mb-1 block">Client</span><select className="form-input w-full py-2" value={moduleHealthClientId} onChange={(event) => setModuleHealthClientId(event.target.value)}><option value="all">All Clients</option>{state.clients.map((client) => <option key={client.clientId} value={client.clientId}>{client.clientName}</option>)}</select></label>
        </div>
        <div className="overflow-x-auto">
          <div className="max-h-[321px] min-w-[760px] overflow-y-auto [scrollbar-color:rgba(34,211,238,0.45)_rgba(255,255,255,0.04)]">
            <table className="w-full table-fixed text-sm">
              ...
              <tbody>{visibleModuleRows.map((row, index) => <tr key={row.moduleName} className="h-[54px] border-b border-white/10 hover:bg-white/[0.04]"><td className="px-3 py-3"><span className="font-medium text-white">{row.moduleName}</span><span className="mt-0.5 block text-xs font-normal text-slate-500">{index + 1} of {visibleModuleRows.length}</span></td><td className="px-3 py-3">{row.disabledClients === 0 ? <span className="text-emerald-200">Enabled</span> : moduleHealthClient ? <span className="text-rose-200">Disabled</span> : <span className="text-amber-200">{row.enabledClients} enabled / {row.disabledClients} disabled</span>}</td><td className="px-3 py-3"><StatusBadge status={row.status} /></td><td className="px-3 py-3 text-slate-400">{row.lastUpdated}</td><td className="px-3 py-3 text-right"><button className="form-button-subtle py-1 text-xs" onClick={() => navigate(`/platform/modules/${slug(row.moduleName)}${moduleHealthClient ? `?clientId=${moduleHealthClient.clientId}` : ''}`)}>View</button></td></tr>)}</tbody>
            </table>
