...
export function PlatformDashboardPage() {
  ...
  const moduleRows = platformModules.map((moduleName, index) => ({
    ...
    enabledClients: state.clients.filter((client) => client.enabledModules.includes(moduleName) || moduleName === 'Admin').length,
    disabledClients: state.clients.filter((client) => client.disabledModules.includes(moduleName)).length,
    status: index === 8 ? 'Attention Needed' : 'Healthy',
    lastUpdated: '21 Jun 2026 09:00',
  }));
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard label="Module Health" value={`${moduleRows.filter((row) => row.status === 'Healthy').length}/${moduleRows.length}`} helper="Platform modules healthy" icon={<Activity className="h-5 w-5" />} onClick={() => navigate(`/platform/modules/${slug(moduleRows[0].moduleName)}`)} />
