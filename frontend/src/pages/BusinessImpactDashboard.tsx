  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <SavingsCard label="Total money saved" value={formatMetricValue(totals.moneySaved, 'INR')} helper={`${totals.improved} improved metrics`} icon={<Banknote className="h-5 w-5" />} onClick={() => openMetric(completed[0])} />
        <SavingsCard label="Total cost avoided" value={formatMetricValue(totals.costAvoided, 'INR')} helper="Risk and loss prevention" icon={<ShieldCheck className="h-5 w-5" />} onClick={() => openMetric(completed[1])} />
