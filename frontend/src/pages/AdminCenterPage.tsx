      ...
      <div className="grid gap-5 xl:grid-cols-[245px_minmax(0,1fr)]">
        <aside className="h-fit rounded-2xl border border-white/10 bg-slate-950/35 p-2 xl:sticky xl:top-24">
          ...
          <nav className="grid gap-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-1">
            <NavLink to="/admin/users" className={({ isActive }) => adminLinkClass(isActive)}><UsersRound className="h-4 w-4" />User Management</NavLink>
...
function CompanyProfile({ user }: { user: RuntimeUser }) {
  ...
  return <div className="space-y-5"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Company Users" value={users.length} helper={`${users.filter((item) => item.status === 'Active').length} active`} /><StatCard label="Applications" value={client.enabledApplications.length} helper="High-level products" /><StatCard label="Modules" value={client.enabledModules.length} helper={`${client.disabledModules.length} platform disabled`} /><StatCard label="Health Score" value="91%" helper="Healthy governance" /></div><Panel title="Company Profile" description={user.role === 'super_admin' ? 'Super Admin can update the client through Client Administration.' : 'Company profile is read-only for Company Admin.'} action={user.role === 'super_admin' ? <button className="form-button-primary" onClick={() => navigate(`/admin/clients/${client.clientId}/edit`)}>Edit Company</button> : undefined}><dl className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{fields.map(([label, value]) => <div key={label} className="rounded-xl border border-white/10 bg-slate-950/30 p-4"><dt className="text-xs uppercase tracking-[0.1em] text-slate-500">{label}</dt><dd className="mt-2 font-medium text-white">{value}</dd></div>)}</dl></Panel><Panel title="Company Access Summary" description="Products and capabilities available in the current client context."><div className="grid gap-4 lg:grid-cols-2"><TagList title="Enabled Applications" items={client.enabledApplications} /><TagList title="Enabled Modules" items={client.enabledModules} /></div></Panel><div className="flex flex-wrap gap-3"><button className="form-button-subtle">View Subscription</button><button className="form-button-subtle" onClick={() => navigate('/admin/modules')}>View Module Usage</button><button className="form-button-subtle" onClick={() => navigate(`/admin/clients/${client.clientId}/health`)}>View Health Score</button></div></div>;
}
