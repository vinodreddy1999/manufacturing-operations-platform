...
import { canPerformAction, canReadAuditLogs } from '../lib/rbac';
...
export function AdminCenterPage({ section, user }: { section: AdminSection; user: RuntimeUser }) {
  const allowedNavigation = adminNavigation.filter((item) => canViewAdminSection(user, item.section));
  if (!allowedNavigation.some((item) => item.section === section)) {
    return <Navigate to={`/admin/${allowedNavigation[0]?.section ?? 'company'}`} replace />;
  }

  return (
    <div className="space-y-4">
      <ModuleNavigationTabs items={allowedNavigation.map((item) => ({ ...item, path: `/admin/${item.section}` }))} dashboardPath="/admin/company" />
...
function RoleManagement({ user }: { user: RuntimeUser }) {
  ...
  return <div className="space-y-5"><Panel title="Roles" description="Business roles, scope, assigned users, and allowed access." action={canPerformAction(user, 'create') ? <button className="form-button-primary" onClick={() => setCreateOpen((value) => !value)}>Create Role</button> : undefined}><AdminTable headers={['Role ID','Role Name','Scope','Description','Assigned Users','Applications','Modules','Status']}>{rows.map((row) => <tr key={row.id} className="border-b border-white/10"><Cell accent>{row.id}</Cell><Cell strong>{row.name}</Cell><Cell>{row.scope}</Cell><Cell>{row.description}</Cell><Cell>{row.users}</Cell><Cell>{row.applications}</Cell><Cell>{row.modules}</Cell><Cell><StatusBadge status={row.status} /></Cell></tr>)}</AdminTable></Panel>{createOpen ? <Panel title="Create Role" description="Company Admin roles are always restricted to the active company."><div className="grid gap-3 md:grid-cols-2"><Field label="Role Name"><input className="form-input mt-1 w-full" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></Field><Field label="Scope"><select className="form-input mt-1 w-full" value={form.scope} disabled={user.role !== 'super_admin'} onChange={(event) => setForm({ ...form, scope: event.target.value })}>{['Platform','Company','Plant','Warehouse','Department'].filter((scope) => user.role === 'super_admin' || scope === 'Company').map((scope) => <option key={scope}>{scope}</option>)}</select></Field><Field label="Description" className="md:col-span-2"><textarea className="form-input mt-1 min-h-24 w-full" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></Field></div><div className="mt-5 space-y-4"><Suspense fallback={<div className="rounded-2xl border border-white/10 bg-slate-950/30 p-4 text-sm text-slate-400">Loading access options...</div>}><LazyMultiSelectAccessGrid title="Allowed Applications" items={selectedClient?.enabledApplications ?? platformApplications} selectedItems={form.applications} onChange={(applications) => setForm({ ...form, applications })} searchPlaceholder="Search applications..." columns={3} /><LazyMultiSelectAccessGrid title="Allowed Modules" items={selectedClient?.enabledModules ?? platformModules} selectedItems={form.modules} onChange={(modules) => setForm({ ...form, modules })} searchPlaceholder="Search modules..." columns={3} /></Suspense><ChoiceRow title="Permissions" items={adminPermissions} selected={form.permissions} onChange={(permissions) => setForm({ ...form, permissions })} /></div><div className="mt-5 flex justify-end gap-2"><button className="form-button-subtle" onClick={() => setCreateOpen(false)}>Cancel</button><button className="form-button-primary" onClick={createRole}>Save Role</button></div></Panel> : null}</div>;
}
