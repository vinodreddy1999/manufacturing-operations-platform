...
export function ImpactDrilldown({ metric }: { metric: ImpactMetric }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3"><BeforeAfterCard metric={metric} onClick={() => undefined} /><SavingsCard label="Financial impact" value={formatMetricValue(metric.financialImpact, 'INR', currency)} helper={metric.operationalImpact} /><SavingsCard label="Responsible owner" value={metric.responsibleOwner} helper={metric.responsibleDepartment} icon={<UserRound className="h-5 w-5" />} /></div>
      <div className="grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
        ...
        <Panel title="Metric context" description="Calculation and accountability metadata"><dl className="space-y-3 text-sm">{[['Previous period', metric.previousPeriod], ['Current period', metric.currentPeriod], ['Difference', formatMetricValue(metric.difference, metric.unit, currency)], ['Percentage change', `${metric.percentageChange}%`], ['Improvement rule', `${metric.improvementWhen} is better`], ['Status', metric.status], ['Last updated', metric.lastUpdated]].map(([label, value]) => <div key={label} className="flex justify-between gap-4 border-b border-white/10 pb-3"><dt className="text-slate-400">{label}</dt><dd className="text-right font-medium text-white">{value}</dd></div>)}</dl></Panel>
      </div>
      <Panel title="Related records" description="Operational records contributing to this impact result"><ScrollableTableFrame count={metric.relatedRecords.length}><table className="min-w-full text-sm"><thead><tr className="border-b border-white/10 text-left text-xs uppercase tracking-[0.1em] text-slate-500"><th className="px-3 py-3">Record</th><th className="px-3 py-3">Description</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Impact</th></tr></thead><tbody>{metric.relatedRecords.map((record) => <tr key={record.id} className="border-b border-white/10"><td className="px-3 py-3 font-medium text-cyan-200">{record.id}</td><td className="px-3 py-3 text-slate-200">{record.name}</td><td className="px-3 py-3 text-slate-300">{record.status}</td><td className="px-3 py-3 text-white">{record.value}</td></tr>)}</tbody></table></ScrollableTableFrame></Panel>
