  ...
  return (
    <button type="button" onClick={onClick} className="group w-full rounded-2xl border border-white/10 bg-slate-900/55 p-4 text-left transition hover:border-cyan-300/35 hover:bg-slate-800/65">
      ...
      <div className="mt-4 flex items-center gap-3">
        <div className="min-w-0 flex-1"><p className="text-[11px] uppercase tracking-[0.12em] text-slate-500">{metric.previousPeriod}</p><p className="mt-1 truncate text-lg font-semibold text-slate-300">{formatMetricValue(metric.previousValue, metric.unit)}</p></div>
        <ArrowRight className="h-4 w-4 shrink-0 text-slate-500" />
        <div className="min-w-0 flex-1 text-right"><p className="text-[11px] uppercase tracking-[0.12em] text-slate-500">{metric.currentPeriod}</p><p className="mt-1 truncate text-lg font-semibold text-white">{formatMetricValue(metric.currentValue, metric.unit)}</p></div>
      </div>
