...
export function StatCard({ label, value, helper, icon, onClick, accent = 'blue' }: StatCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`enterprise-card group min-h-[118px] w-full p-token-4 text-left transition ${onClick ? 'hover:border-cyan-300/40 hover:bg-slate-800/70' : ''}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="enterprise-eyebrow text-slate-400">{label}</p>
